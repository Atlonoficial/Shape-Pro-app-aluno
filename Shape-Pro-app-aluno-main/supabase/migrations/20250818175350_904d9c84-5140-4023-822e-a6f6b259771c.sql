...
DROP FUNCTION IF EXISTS public.list_available_slots_improved(uuid, date, date, integer);
...
CREATE OR REPLACE FUNCTION public.list_available_slots_improved(p_teacher_id uuid, p_start_date date, p_end_date date, p_slot_minutes integer DEFAULT NULL::integer)
 RETURNS TABLE(slot_date date, slot_start timestamp with time zone, slot_end timestamp with time zone, slot_minutes integer, teacher_id uuid)
 LANGUAGE plpgsql
 SECURITY DEFINER
