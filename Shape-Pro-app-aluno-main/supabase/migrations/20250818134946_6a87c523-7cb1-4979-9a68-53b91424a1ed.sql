-- Debug and fix the list_available_slots_improved function
...
begin
  ...
  if not found then
    ...
    raise notice 'Found booking settings: min_advance=%, same_day=%, visibility=%', 
      booking_settings.minimum_advance_hours, 
  ...
  raise notice 'Current time: %, Min booking time: %', now(), min_booking_time;
    ...
    if has_special_hours then
      ...
      loop
        ...
        if not exists (
          ...
          raise notice 'Returning special slot: % to %', s, s + make_interval(mins => step);
          return next;
        end if;
      end loop;
    else
      ...
      loop
        ...
        loop
          ...
          if not exists (
            ...
            slot_start := s;
            slot_end := s + make_interval(mins => step);
            raise notice 'Returning slot: % to %', s, s + make_interval(mins => step);
            return next;
          else
