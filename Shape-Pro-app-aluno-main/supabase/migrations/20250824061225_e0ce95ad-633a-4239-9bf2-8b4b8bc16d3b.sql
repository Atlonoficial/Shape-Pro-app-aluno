-- Criar função para adicionar rotações de refeições (variações)
CREATE OR REPLACE FUNCTION create_meal_rotation(
...
-- Função melhorada para obter refeições do dia
...
BEGIN
  ...
  WHERE np.assigned_to = p_user_id
