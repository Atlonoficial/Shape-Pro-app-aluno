-- Fix timezone handling in list_available_slots_improved function - CORRECT VERSION
...
begin
  ...
  -- Get current time in Brazil timezone (FIXED)
  ...
  -- Calculate minimum booking time based on advance minutes using Brazil timezone (FIXED)
  ...
  while iter_date <= p_end_date loop
    ...
    -- Skip dates that are too far in advance or don't allow same day booking (FIXED)
    ...
    if has_special_hours then
      ...
      while s + make_interval(mins => step) <= series_end loop
        ...
        -- Check if slot meets minimum advance time requirement (FIXED: both in Brazil timezone)
