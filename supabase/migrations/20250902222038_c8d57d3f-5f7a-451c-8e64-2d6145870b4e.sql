-- Limpar registros duplicados de gamificação (correção do operador)
...
WITH duplicates AS (
  SELECT id,
         ROW_NUMBER() OVER (
           ...
           CASE 
             WHEN activity_type = 'meal_logged' THEN COALESCE(metadata->>'meal_id', '') || '_' || COALESCE(metadata->>'date', '')
             WHEN activity_type = 'progress_logged' THEN COALESCE(metadata->>'progress_type', '')
             ELSE activity_type
           END
...
DROP TRIGGER IF EXISTS auto_award_meal_points_trigger_v2 ON meal_logs;
DROP TRIGGER IF EXISTS auto_award_progress_points_trigger_v2 ON progress;
...
-- Sistema de gamificação atualizado com prevenção completa de duplicações