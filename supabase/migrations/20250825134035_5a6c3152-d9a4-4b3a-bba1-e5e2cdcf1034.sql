...
-- Recalcular pontos totais por usuário
...
SELECT 
  ...
  public.calculate_user_level(COALESCE(SUM(ga.points_earned), 0)) as level,
  COALESCE(MAX(DATE(ga.created_at)), CURRENT_DATE) as last_activity_date,
  0 as current_streak, -- Será recalculado pelos hooks
  0 as longest_streak   -- Será recalculado pelos hooks
...
BEGIN
  ...
  SELECT EXISTS(
    ...
    AND ga.created_at > (now() - interval '1 minute') -- Janela de 1 minuto para evitar duplicações
...
-- FASE 5: Recriar triggers usando a nova função
CREATE TRIGGER auto_award_meal_points_trigger_v2
  AFTER INSERT OR UPDATE ON meal_logs
  FOR EACH ROW
  WHEN (NEW.consumed = true AND (OLD IS NULL OR OLD.consumed = false))
...
-- FASE 6: Criar funções de trigger melhoradas
...
-- FASE 7: Adicionar constraint para prevenir duplicações futuras
ALTER TABLE gamification_activities 
ADD CONSTRAINT unique_activity_per_minute 
UNIQUE (user_id, activity_type, created_at, metadata);

-- FASE 8: Habilitar realtime para tabelas relevantes
