-- Fix the list_available_slots_improved function to properly handle canceled appointments
-- and improve timezone/advance hours logic
  ...
  while iter_date <= p_end_date loop
    ...
    if has_special_hours then
      ...
      loop
        ...
        -- Fixed: Only check for non-cancelled appointments
        if not exists (
          select 1 from public.appointments a
          where a.teacher_id = p_teacher_id
            and a.status != 'cancelled'  -- Explicitly exclude cancelled appointments
...
-- Clean up inconsistent appointment data - remove duplicate appointments at same time
-- Keep only the most recent appointment when there are conflicts
WITH duplicates AS (
  SELECT 
    teacher_id, 
    scheduled_time,
  ...
  GROUP BY teacher_id, scheduled_time 
  HAVING COUNT(*) > 1
)
UPDATE appointments 
SET status = 'cancelled', 
    cancellation_reason = 'Duplicate appointment cleaned up automatically',
    cancelled_at = now(),
    cancelled_by = teacher_id
FROM duplicates 
WHERE appointments.id = ANY(duplicates.ids[2:]) -- Keep the first (most recent), cancel the rest
  AND appointments.teacher_id = '0d5398c2-278e-4853-b980-f36961795e52';