...
-- Clean up duplicate appointments - cancel older duplicates keeping the newest one
UPDATE appointments 
SET status = 'cancelled', 
    cancellation_reason = 'Duplicate appointment cleaned up automatically',
    cancelled_at = now(),
    cancelled_by = '0d5398c2-278e-4853-b980-f36961795e52'
WHERE id IN (
  SELECT id 
  FROM (
    SELECT 
      id,
      ROW_NUMBER() OVER (
        PARTITION BY teacher_id, scheduled_time 
